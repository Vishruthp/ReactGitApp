...
class FetchDemo extends React.Component {
    ...
    search(query = "tertis") {
        var url = 'https://api.github.com/search/repositories?q=' + query + '&order=desc';
    ...
    SelectRepo(id) {
        ...
        if (selectedRepo) {
            ...
            onClick = {
                (e) => { this.reloaditems(); } }
            key = { selectedRepo.id } > Back < /button> 
			</ToggleDisplay> 
				</div>;
        }
    ...
    render() {
        var names = lodash.map(this.state.items, (item) => {
            return <li onClick = {
                (e) => { this.SelectRepo(item.id); } }
            key = { item.id } > < a href = "#" > { item.name } < /a></li > ;
        });
        var Stars = < button onClick = {
            (e) => { this.StarSort(); } } > Stars < /button>;
        var Watch = < button onClick = {
            (e) => { this.WatchSort(); } } > Watch < /button>;
        var Forks = < button onClick = {
            (e) => { this.ForksSort(); } } > Forks < /button>;
        var Name = < button onClick = {
            (e) => { this.NameSort(); } } > Name < /button>;
        var User = < button onClick = {
            (e) => { this.UserSort(); } } > User < /button>;

        return (

            <div >
            <ToggleDisplay show = { this.state.show } >
            <h1 > GitHub Repos < /h1>  <input ref = "query" onChange = {(e) => { this.updateSearch(); } } type = "text" defaultValue = "tertis" / >
            <br/>
            <p> <strong> SORT BY </strong></p> { Stars } { Watch } { Forks } { Name } { User } <
            ul id = "RepoList" > { names } < /ul> </ToggleDisplay> 
			</div>
        );
      }
    }
